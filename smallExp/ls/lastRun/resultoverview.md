...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` N:/𾟽 ```  <br> ``` r:/ < /?􏰜 ```  <br> ``` file://[::adcf:cedb:252.254.202.255] ```  <br> ``` file:/// < /,椭 ```  <br> ``` ftp://[::] < /? ```  <br> ``` file:/// < .%2e/ ```  <br> ``` file:/// < //[::]/%2e%2e//ꮎ ```  <br> ``` V:/%2e./ ```  <br> ``` file:/// < /Ⅲ ```  <br> ``` file:/// < /%60ᦀ ```  <br> ``` http://[::] < /?ﻃ ```  <br> ``` file:/// < 򰌩 ```  <br> ``` file:/// < 򰠧 ```  <br> ``` http://[::]/)￺ ```  <br> ``` ws://[::] < /?ﻶ ```  <br> ``` http://[::]/%2e%2e//﷎ ```  <br> ``` K:/ < /// ```  <br> ``` http://[::]?￈ ```  <br> ``` file:/// < ///%2e//񓀯 ```  <br> ``` I:/ < ///﷎ ```  <br> ``` file:/// < ///[] ```  <br> ``` file:/// < ///%2e.//򃗸 ```  <br> ``` T:/? ```  <br> ``` https://} ```  <br> ``` a:///'ﺩ ```  <br> ``` file://Ćm ```  <br> ``` c:/ < /=% ```  <br> ``` file:/// < //{n ```  <br> ``` file:///#񅏼 ```  <br> ``` G://` ```  <br> 
...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: wss ```  |  ``` wss://254.57.253.254 ```  <br> ``` wss://[::] < //[::4.250.250.252] ```  <br> ``` wss://[::fe8d:252.250.253.255] < / ```  <br> ``` wss://-U ```  <br> ``` wss://[::] < //[::cbcc:dabe:254.251.21.255] ```  <br> ``` wss://[::254.255.251.61] < / ```  <br> ``` wss://[f0ca::bafe:251.253.250.251] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: ws ```  |  ``` ws://[::7.250.250.251] ```  <br> ``` ws://[::] < /?ﻶ ```  <br> ``` ws://[::]/%2e ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: g ```  |  ``` G:/ < //[::58.254.250.252] ```  <br> ``` G:/ < //[::253.250.31.250] ```  <br> ``` g:/ ```  <br> ``` G://` < / ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: i ```  |  ``` i://[::]:65301 < / ```  <br> ``` I:/ < ///﷎ ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: c ```  |  ``` c:/ < //[::aeed:251.254.223.255] ```  <br> ``` c:/ < ///j:/ ```  <br> ``` c:/ < /=% ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: w ```  |  ``` w:/J:/ ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: k ```  |  ``` K:/ < /// ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: t ```  |  ``` T:/? < / ```  <br> ``` T://[dd2a:beca::ebfb:caaf:250.254.251.252] < / ```  <br> 
...
### chromium
 ...
 Exception Type | URLs 
 --- | --- 
 ``` invalid URL ```  |  ``` I:/ < ///﷎ ```  <br> ``` T://[dd2a:beca::ebfb:caaf:250.254.251.252] < / ```  <br> ``` i://[::]:65301 < / ```  <br> ``` K:/ < /// ```  <br> ``` file:/// < //;v ```  <br> ``` G://` < / ```  <br> ``` a:///'ﺩ < / ```  <br> ``` S://[6fec::cbbc:ebca:255.251.254.253] < / ```  <br> 
...
### Ruby
 ...
 ``` URI must be ascii only "/,\u692D" ```  |  ``` file:/// < /,椭 ```  <br> 
 ...
 ``` URI must be ascii only "/\u2162" ```  |  ``` file:/// < /Ⅲ ```  <br> 
 ``` URI must be ascii only "/%60\u1980" ```  |  ``` file:/// < /%60ᦀ ```  <br> 
 ``` bad URI(is not URI?): //[::58.254.250.252] ```  |  ``` G:/ < //[::58.254.250.252] ```  <br> 
 ``` bad URI(is not URI?): //[::4.250.250.252] ```  |  ``` wss://[::] < //[::4.250.250.252] ```  <br> 
 ...
 ``` bad URI(is not URI?): //[::aeed:251.254.223.255] ```  |  ``` c:/ < //[::aeed:251.254.223.255] ```  <br> 
 ...
 ``` bad URI(is not URI?): //[::254.254.250.4] ```  |  ``` https://[::] < //[::254.254.250.4] ```  <br> 
...
## URL Comparison 
 ...
 ``` file://[::adcf:cedb:252.254.202.255] ```  | C <br>
 ``` file:/// < /,椭 ```  | C <br>Ruby <br>
 ...
 ``` G:/ < //[::58.254.250.252] ```  | Java <br>Ruby <br>
 ``` i://[::]:65301 < / ```  | Java <br>chromium <br>
 ``` wss://[::] < //[::4.250.250.252] ```  | Java <br>Ruby <br>
 ...
 ``` c:/ < //[::aeed:251.254.223.255] ```  | Java <br>Ruby <br>
...
### firefox
 ...
 ``` i://[::]:65301 < / ```  | host |  ``` :: ```  |  ``` NS_ERROR_FAILURE 2147500037 ``` 
 ``` A:/ < //[cab7::ebfe] ```  | host |  ``` cab7::ebfe ```  |  ``` NS_ERROR_FAILURE 2147500037 ``` 
 ``` file://[::deaf:abbe:254.20.254.252] ```  | host |  ``` ::deaf:abbe:fe14:fefc ```  |  ```  ``` 
 ``` file:///%2e/%2e ```  | filePath |  ``` / ```  |  ``` /%2e ``` 
 ``` file://[::adcf:cedb:252.254.202.255] ```  | host |  ``` ::adcf:cedb:fcfe:caff ```  |  ```  ``` 
 ``` file://[::bb1d:250.252.255.250] ```  | host |  ``` ::bb1d:fafc:fffa ```  |  ```  ``` 
 ``` file://[::cebf:eedc:254.191.250.251] ```  | host |  ``` ::cebf:eedc:febf:fafb ```  |  ```  ``` 
 ``` c:/ < //[::aeed:251.254.223.255] ```  | host |  ``` ::aeed:fbfe:dfff ```  |  ``` NS_ERROR_FAILURE 2147500037 ``` 
