...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` http://[::] < @𸻾 ```  <br> ``` file:/// < /#Ｓ ```  <br> ``` B://[::deee:202.255.253.255] ```  <br> ``` https://[::] < /#[ ```  <br> ``` Z:/ < /#% ```  <br> ``` file:///? ```  <br> ``` wss://[::]/./ ```  <br> ``` V:/ < /[ ```  <br> ``` file:/// < //{y ```  <br> ``` ws://[::]/;횘 ```  <br> ``` file:/// < /&ఒ ```  <br> ``` N:/ < /&￠ ```  <br> ``` file:///%7b￧ ```  <br> ``` file://[::]/ꪸ ```  <br> ``` file:///#󥓈 ```  <br> ``` file:/// < /? ```  <br> ``` file://`$ ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` file:///鍕 ```  <br> ``` file:/// < //"* ```  <br> ``` file:/// < ︢ ```  <br> ``` ws://[::] < /|, ```  <br> ``` ftp://[::] < / ```  <br> ``` file:/// < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` L:/ < ///.// ```  <br> ``` file:/// < %7dＥ ```  <br> ``` wss://Gſ ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` file://[::]/%2e%2e/󬱰 ```  <br> ``` ws://[::]/,% ```  <br> ``` file:/// < 햐 ```  <br> ``` H:/#􏨊 ```  <br> ``` file:/// < .%2e///[ ```  <br> ``` file:/// < /.///ᶓ ```  <br> ``` file://" ```  <br> ``` file:///#틣 ```  <br> ``` http://[::] < ퟺ ```  <br> ``` A:/#隤 ```  <br> ``` S:/(ﻴ ```  <br> ``` file:/// < //[::]/.//㶭 ```  <br> ``` https://[::] < ퟾ ```  <br> ``` ftp://[::eecc:255.252.255.207] ```  <br> ``` https://[::] < //%@) ```  <br> ``` Z:///,􏛖 ```  <br> ``` h:/񻿼 ```  <br> ``` ws://[::]/)︒ ```  <br> ``` J:/ ```  <br> ``` file:/// < /# ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` M:/ < /+ ```  <br> ``` wss://[::] < /𰵮 ```  <br> ``` file:///^ ```  <br> ``` https://[::] < 􌧄 ```  <br> ``` V:///]! ```  <br> ``` file:/// < //[::]/ﺑ ```  <br> ``` http://[::] < %2e//󠱎 ```  <br> ``` B:/ < /#︓ ```  <br> ``` http://[::] < //󢯼@= ```  <br> ``` https://[::] < //M@` ```  <br> ``` G:/%7bᡚ ```  <br> ``` http://[::]?ퟻ ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> ``` http://[::]/%60𓜎 ```  <br> ``` ftp://[::] < ﹐ ```  <br> ``` http://[::]?沫 ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` ftp://[::] < 􏟃 ```  <br> ``` file:///#⮃ ```  <br> ``` p:/?ﺖ ```  <br> ``` wss://[::] < //ſ3 ```  <br> ``` file:///'＿ ```  <br> ``` file://[::]/酜 ```  <br> ``` file:/// < //}` ```  <br> 
...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: t ```  |  ``` t:/ < //T ```  <br> ``` T:/ < //%2e ```  <br> ``` T:/ < //[::4aba:251.250.254.255] ```  <br> ``` t:/ < //[cfae::dbff] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: y ```  |  ``` y://[::fdde:223.252.254.253] ```  <br> ``` Y:/ < //O ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` y:/ < //[aaf3:ccdc:cfbd:ecef:acfe:efdb:251.254.252.251] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: e ```  |  ``` e:/ < //[::7bce:fdcc] ```  <br> ``` e:/ < %2e////= ```  <br> ``` E:/%60( ```  <br> ``` E:/N:/ ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: l ```  |  ``` L:/ < ///.// ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: o ```  |  ``` o:/// < / ```  <br> ``` o://[::5bde] < / ```  <br> ``` o://[dcbb:ddbb::ddbb] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: s ```  |  ``` s:/ < //D ```  <br> ``` s:/ < /?= ```  <br> ``` s:/a:/ < / ```  <br> ``` S:/(ﻴ ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: h ```  |  ``` H:/#􏨊 < / ```  <br> ``` h:/񻿼 ```  <br> ``` H:/ < //[::7afa:254.250.252.253] ```  <br> ``` h:/ < //[::eca2] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: w ```  |  ``` w:/ < //%5d ```  <br> ``` w://[::]:65516 ```  <br> ``` W:/ < //[db4d::fdfd:cecb:250.254.251.254] ```  <br> ``` w:/ < //[::aeae:fcae:86.253.250.253] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: q ```  |  ``` q:/ < //[::fcaa:fddb:255.251.250.52] ```  <br> 
...
### chromium
 ...
 Exception Type | URLs 
 --- | --- 
 ``` invalid URL ```  |  ``` F:/ < //~! ```  <br> ``` J:/ < //; ```  <br> ``` X://M < / ```  <br> ``` v://[ceda:eaab:cafc:afbb:bfac:fdaf:254.253.253.253] < / ```  <br> ``` B://[::deee:202.255.253.255] < / ```  <br> ``` z://[::dfbe:afea:251.254.251.254] < / ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` https://;~ ```  <br> ``` o:/// < / ```  <br> ``` z://[abce::] < / ```  <br> ``` w:/ < //%5d ```  <br> ``` o://[::5bde] < / ```  <br> 
