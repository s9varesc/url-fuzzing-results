...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` http://[::] < @𸻾 ```  <br> ``` file:/// < /#Ｓ ```  <br> ``` B://[::deee:202.255.253.255] ```  <br> ``` file:///? ```  <br> ``` wss://[::]/./ ```  <br> ``` file:/// < //{y ```  <br> ``` file:///%7b￧ ```  <br> ``` file://[::]/ꪸ ```  <br> ``` file:///#󥓈 ```  <br> ``` file://`$ ```  <br> ``` ftp://[::] < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` file:/// < %7dＥ ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` file://[::]/%2e%2e/󬱰 ```  <br> ``` file:/// < 햐 ```  <br> ``` file:/// < /.///ᶓ ```  <br> ``` file://" ```  <br> ``` http://[::] < ퟺ ```  <br> ``` A:/#隤 ```  <br> ``` S:/(ﻴ ```  <br> ``` https://[::] < //%@) ```  <br> ``` M:/ < /+ ```  <br> ``` V:///]! ```  <br> ``` http://[::] < //󢯼@= ```  <br> ``` https://[::] < //M@` ```  <br> ``` http://[::]/%60𓜎 ```  <br> ``` ftp://[::] < ﹐ ```  <br> ``` http://[::]?沫 ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` file:///#⮃ ```  <br> ``` file://[::]/酜 ```  <br> 
...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: p ```  |  ``` P:/. ```  <br> ``` P:/ < //[::fbaa:ceec:250.254.149.251] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: v ```  |  ``` V:/%23 < / ```  <br> ``` V:///]! ```  <br> ``` v://[ceda:eaab:cafc:afbb:bfac:fdaf:254.253.253.253] < / ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: s ```  |  ``` s:/ < //D ```  <br> ``` S:/(ﻴ ```  <br> 
...
### Ruby
 ...
 ``` bad URI(is not URI?):  ```  |  ``` file://`$ ```  <br> ``` file://" ```  <br> ``` V:///]! ```  <br> 
