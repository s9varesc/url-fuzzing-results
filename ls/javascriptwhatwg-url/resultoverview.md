...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` http://[::] < @𸻾 ```  <br> ``` file:/// < /#Ｓ ```  <br> ``` B://[::deee:202.255.253.255] ```  <br> ``` https://[::] < /#[ ```  <br> ``` Z:/ < /#% ```  <br> ``` file:///? ```  <br> ``` wss://[::]/./ ```  <br> ``` file:/// < //{y ```  <br> ``` ws://[::]/;횘 ```  <br> ``` file:/// < /&ఒ ```  <br> ``` N:/ < /&￠ ```  <br> ``` file:///%7b￧ ```  <br> ``` file://[::]/ꪸ ```  <br> ``` file:///#󥓈 ```  <br> ``` file:/// < /? ```  <br> ``` file://`$ ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` file:///鍕 ```  <br> ``` file:/// < //"* ```  <br> ``` file:/// < ︢ ```  <br> ``` ftp://[::] < / ```  <br> ``` file:/// < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` file:/// < %7dＥ ```  <br> ``` wss://Gſ ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` file://[::]/%2e%2e/󬱰 ```  <br> ``` ws://[::]/,% ```  <br> ``` file:/// < 햐 ```  <br> ``` file:/// < .%2e///[ ```  <br> ``` file:/// < /.///ᶓ ```  <br> ``` file://" ```  <br> ``` http://[::] < ퟺ ```  <br> ``` A:/#隤 ```  <br> ``` S:/(ﻴ ```  <br> ``` file:/// < //[::]/.//㶭 ```  <br> ``` ftp://[::eecc:255.252.255.207] ```  <br> ``` https://[::] < //%@) ```  <br> ``` h:/񻿼 ```  <br> ``` J:/ ```  <br> ``` file:/// < /# ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` M:/ < /+ ```  <br> ``` wss://[::] < /𰵮 ```  <br> ``` file:///^ ```  <br> ``` https://[::] < 􌧄 ```  <br> ``` V:///]! ```  <br> ``` file:/// < //[::]/ﺑ ```  <br> ``` http://[::] < %2e//󠱎 ```  <br> ``` B:/ < /#︓ ```  <br> ``` http://[::] < //󢯼@= ```  <br> ``` https://[::] < //M@` ```  <br> ``` G:/%7bᡚ ```  <br> ``` http://[::]?ퟻ ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> ``` http://[::]/%60𓜎 ```  <br> ``` ftp://[::] < ﹐ ```  <br> ``` http://[::]?沫 ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` ftp://[::] < 􏟃 ```  <br> ``` file:///#⮃ ```  <br> ``` p:/?ﺖ ```  <br> ``` wss://[::] < //ſ3 ```  <br> ``` file:///'＿ ```  <br> ``` file://[::]/酜 ```  <br> ``` file:/// < //}` ```  <br> 
...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: ws ```  |  ``` ws://[::]/;횘 < / ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` ws://[::] < //[::babe:dcfb:ed7d:abbd] ```  <br> ``` ws://[::]/,% < / ```  <br> ``` ws://[::] < //[::daaf:cfca:255.22.254.252] ```  <br> ``` ws://[::] < V ```  <br> ``` ws://[caf7::255.252.252.251] ```  <br> ``` ws://[::fdca:251.251.254.83] < / ```  <br> ``` ws://[::]?! < / ```  <br> ``` ws://[::cfed:254.251.89.255] ```  <br> ``` ws://[::] < /[풂 ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: t ```  |  ``` t:/ < //T ```  <br> ``` T:/ < //%2e ```  <br> ``` t:/ < //[cfae::dbff] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: v ```  |  ``` v:/ < //[::babd:244.252.253.254] ```  <br> ``` V://[::afab:bc8d:bbdc] ```  <br> ``` V:/%23 < / ```  <br> ``` V:///]! ```  <br> ``` v://[ceda:eaab:cafc:afbb:bfac:fdaf:254.253.253.253] < / ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: w ```  |  ``` w:/ < //%5d ```  <br> ``` W:/ < //[db4d::fdfd:cecb:250.254.251.254] ```  <br> ``` w:/ < //[::aeae:fcae:86.253.250.253] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: y ```  |  ``` Y:/ < %7b￸ ```  <br> ``` y:/ < //[aaf3:ccdc:cfbd:ecef:acfe:efdb:251.254.252.251] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: q ```  |  ``` q:/ < //[::fcaa:fddb:255.251.250.52] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: g ```  |  ``` g:/ < //[::acbe:bece:252.99.251.254] ```  <br> ``` G:/%7bᡚ ```  <br> 
