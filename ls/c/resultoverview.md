...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: a ```  |  ``` A:/ < //[::aafe:d3dc:bddf] ```  <br> ``` A:/#隤 < / ```  <br> ``` a:/ < //%6f ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: ws ```  |  ``` ws://[::]/;횘 < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` ws://[::] < //[::babe:dcfb:ed7d:abbd] ```  <br> ``` ws://[::] < V ```  <br> ``` ws://[caf7::255.252.252.251] ```  <br> ``` ws://[::fdca:251.251.254.83] < / ```  <br> ``` ws://[::]?! < / ```  <br> ``` ws://[::cfed:254.251.89.255] ```  <br> ``` ws://[::] < /[풂 ```  <br> 
