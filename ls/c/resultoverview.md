...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` http://[::] < @𸻾 ```  <br> ``` file:/// < /#Ｓ ```  <br> ``` ftp://[::] < /?򯹨 ```  <br> ``` B://[::deee:202.255.253.255] ```  <br> ``` https://[::] < /#[ ```  <br> ``` Z:/ < /#% ```  <br> ``` file:///? ```  <br> ``` wss://[::]/./ ```  <br> ``` V:/ < /[ ```  <br> ``` file:/// < /% ```  <br> ``` file:/// < //{y ```  <br> ``` file:///#^ ```  <br> ``` F:/ < 쵖 ```  <br> ``` ws://[::]/;횘 ```  <br> ``` file:/// < /&ఒ ```  <br> ``` N:/ < /&￠ ```  <br> ``` o:/ < ￹ ```  <br> ``` file:///%7b￧ ```  <br> ``` https://[::]#ﾛ ```  <br> ``` file://[::]/ꪸ ```  <br> ``` file:///#󥓈 ```  <br> ``` file:/// < /%2e//[ ```  <br> ``` file:/// < /? ```  <br> ``` file://`$ ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` file:///鍕 ```  <br> ``` https://[::]/﫞 ```  <br> ``` file:/// < //"* ```  <br> ``` file:/// < ///|񁣀 ```  <br> ``` file:/// < ︢ ```  <br> ``` ws://[::] < /|, ```  <br> ``` ftp://[::] < / ```  <br> ``` wss://􀕻@+ ```  <br> ``` file:///%7b􏕒 ```  <br> ``` R:/[񂗉 ```  <br> ``` file:/// < / ```  <br> ``` http://[::] < /?텫 ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` L:/ < ///.// ```  <br> ``` file://[::]/../􏳁 ```  <br> ``` ws://[::] < /!އ ```  <br> ``` file:/// < %7dＥ ```  <br> ``` O:/ < %2e%2e/// ```  <br> ``` file:///%%22 ```  <br> ``` file:/// < //ċz ```  <br> ``` wss://Gſ ```  <br> ``` file:/// < //[::]/./鹷 ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` http://[::] < //[::]/%60ａ ```  <br> ``` file://[::]/%2e%2e/󬱰 ```  <br> ``` ws://[::]/,% ```  <br> ``` file:/// < 햐 ```  <br> ``` H:/#􏨊 ```  <br> ``` file:/// < .%2e///[ ```  <br> ``` https://[::]?㡑 ```  <br> ``` http://[::] < /?ퟻ ```  <br> ``` file:/// < /.///ᶓ ```  <br> ``` ftp://[::] < ./񾣩 ```  <br> ``` file://" ```  <br> ``` file:///#틣 ```  <br> ``` http://[::] < ퟺ ```  <br> ``` x:/ < ///'￪ ```  <br> ``` file:/// < /.////쇕 ```  <br> ``` P:/../[ ```  <br> ``` A:/#隤 ```  <br> ``` ws://[::] < /# ```  <br> ``` W:/ < /// ```  <br> ``` https://[::] < 放 ```  <br> ``` S:/(ﻴ ```  <br> ``` t:/ < /$񳗓 ```  <br> ``` file:/// < //[::]/.//㶭 ```  <br> ``` file:/// < /崍 ```  <br> ``` file:///^ ```  <br> ``` https://[::] < ퟾ ```  <br> ``` file:/// < ///=ｿ ```  <br> ``` ftp://[::eecc:255.252.255.207] ```  <br> ``` https://[::] < //%@) ```  <br> ``` H:/ < %2e%2e/𧀷 ```  <br> ``` Z:///,􏛖 ```  <br> ``` h:/񻿼 ```  <br> ``` ws://[::]/)︒ ```  <br> ``` J:/ ```  <br> ``` file:/// < /# ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` z:/ < /ﾚ ```  <br> ``` M:/ < /+ ```  <br> ``` wss://[::]?Ҽ ```  <br> ``` wss://[::] < /𰵮 ```  <br> ``` http://[::]?򅴄 ```  <br> ``` https://[::] < 􌧄 ```  <br> ``` V:///]! ```  <br> ``` file:/// < //[::]/ﺑ ```  <br> ``` ws://@~ ```  <br> ``` H://" ```  <br> ``` http://[::] < %2e//󠱎 ```  <br> ``` B:/ < /#︓ ```  <br> ``` c:/ < /ﮢ ```  <br> ``` http://[::] < //󢯼@= ```  <br> ``` https://[::] < //M@` ```  <br> ``` file:///# ```  <br> ``` G:/%7bᡚ ```  <br> ``` http://[::]?ퟻ ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> ``` http://[::]/%60𓜎 ```  <br> ``` ftp://[::] < ﹐ ```  <br> ``` z:/ < //$} ```  <br> ``` file:///#򮠱 ```  <br> ``` http://[::]?沫 ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` ftp://[::] < 􏟃 ```  <br> ``` file:///#⮃ ```  <br> ``` p:/?ﺖ ```  <br> ``` wss://[::] < //ſ3 ```  <br> ``` file:///'＿ ```  <br> ``` file://}a ```  <br> ``` file://[::]/酜 ```  <br> ``` file:/// < //}` ```  <br> ``` file:/// < //[::]/%2e.//痁 ```  <br> ``` r:/| ```  <br> 
...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: a ```  |  ``` A:/ < //[::aafe:d3dc:bddf] ```  <br> ``` a:/ < //[aebb:cfee:ddba:cbda:fbca:aebc:b6cd:dcba] ```  <br> ``` A:/ < //[::adbf:dedc:252.254.44.255] ```  <br> ``` A:/ < ///%2e%2e//%2e ```  <br> ``` A:/#隤 < / ```  <br> ``` a:/ < //%6f ```  <br> ``` a:/ < //[::251.253.251.215] ```  <br> ``` A:/ < //[::aeb6] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: s ```  |  ``` S:/ < //[::dae9] ```  <br> ``` S://a2 ```  <br> ``` s:/ < //D ```  <br> ``` S:/q:/ < / ```  <br> ``` s:/ < /?= ```  <br> ``` s:/a:/ < / ```  <br> ``` S:/(ﻴ ```  <br> ``` s://' ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: y ```  |  ``` y://[::fdde:223.252.254.253] ```  <br> ``` Y:/ < //O ```  <br> ``` Y:/ < ///S:/ ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` y:/ < //[aaf3:ccdc:cfbd:ecef:acfe:efdb:251.254.252.251] ```  <br> 
...
### chromium
 ...
 Exception Type | URLs 
 --- | --- 
 ``` invalid URL ```  |  ``` F:/ < //~! ```  <br> ``` l://[bffb:ddcd::cbae:dffb:253.250.251.252] < / ```  <br> ``` J:/ < //; ```  <br> ``` X://M < / ```  <br> ``` v://[ceda:eaab:cafc:afbb:bfac:fdaf:254.253.253.253] < / ```  <br> ``` B://[::deee:202.255.253.255] < / ```  <br> ``` C:/ < //;0 ```  <br> ``` q://[daf9:dcce::dbcf:fbdc:255.250.254.252] < / ```  <br> ``` z://[::dfbe:afea:251.254.251.254] < / ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` https://;~ ```  <br> ``` W:/ < /// ```  <br> ``` o:/// < / ```  <br> ``` z://[abce::] < / ```  <br> ``` w:/ < //%5d ```  <br> ``` file://;~ < / ```  <br> ``` o://[::5bde] < / ```  <br> ``` ws://@~ ```  <br> 
...
### Ruby
 ...
 ``` URI must be ascii only "/?\uEE87" ```  |  ``` file:/// < /? ```  <br> 
 ``` bad URI(is not URI?):  ```  |  ``` file://`$ ```  <br> ``` file://" ```  <br> ``` P:/../[ ```  <br> ``` file:///^ ```  <br> ``` V:///]! ```  <br> ``` H://" ```  <br> ``` file://}a ```  <br> 
