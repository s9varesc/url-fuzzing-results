...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` http://[::] < @𸻾 ```  <br> ``` file:/// < /#Ｓ ```  <br> ``` B://[::deee:202.255.253.255] ```  <br> ``` https://[::] < /#[ ```  <br> ``` file:///? ```  <br> ``` wss://[::]/./ ```  <br> ``` file:/// < //{y ```  <br> ``` ws://[::]/;횘 ```  <br> ``` file:/// < /&ఒ ```  <br> ``` N:/ < /&￠ ```  <br> ``` file:///%7b￧ ```  <br> ``` file://[::]/ꪸ ```  <br> ``` file:///#󥓈 ```  <br> ``` file:/// < /? ```  <br> ``` file://`$ ```  <br> ``` file:///鍕 ```  <br> ``` file:/// < ︢ ```  <br> ``` ftp://[::] < / ```  <br> ``` file:/// < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` file:/// < %7dＥ ```  <br> ``` wss://Gſ ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` file://[::]/%2e%2e/󬱰 ```  <br> ``` file:/// < 햐 ```  <br> ``` file:/// < .%2e///[ ```  <br> ``` file:/// < /.///ᶓ ```  <br> ``` file://" ```  <br> ``` http://[::] < ퟺ ```  <br> ``` A:/#隤 ```  <br> ``` S:/(ﻴ ```  <br> ``` ftp://[::eecc:255.252.255.207] ```  <br> ``` https://[::] < //%@) ```  <br> ``` h:/񻿼 ```  <br> ``` J:/ ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` M:/ < /+ ```  <br> ``` wss://[::] < /𰵮 ```  <br> ``` file:///^ ```  <br> ``` https://[::] < 􌧄 ```  <br> ``` V:///]! ```  <br> ``` file:/// < //[::]/ﺑ ```  <br> ``` http://[::] < //󢯼@= ```  <br> ``` https://[::] < //M@` ```  <br> ``` http://[::]?ퟻ ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> ``` http://[::]/%60𓜎 ```  <br> ``` ftp://[::] < ﹐ ```  <br> ``` http://[::]?沫 ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` ftp://[::] < 􏟃 ```  <br> ``` file:///#⮃ ```  <br> ``` p:/?ﺖ ```  <br> ``` file:///'＿ ```  <br> ``` file://[::]/酜 ```  <br> 
...
### Java
 ...
 Exception Type | URLs 
 --- | --- 
 ``` java.net.MalformedURLException: unknown protocol: b ```  |  ``` B://[::deee:202.255.253.255] < / ```  <br> ``` b://s ```  <br> ``` b:/ < 3 ```  <br> ``` b:/ < ///) ```  <br> ``` b:/ < //[fcbf:cbfa::eceb:252.251.253.251] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: ws ```  |  ``` ws://[::]/;횘 < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` ws://[::] < //[::babe:dcfb:ed7d:abbd] ```  <br> ``` ws://[::] < //[::daaf:cfca:255.22.254.252] ```  <br> ``` ws://[::] < V ```  <br> ``` ws://[caf7::255.252.252.251] ```  <br> ``` ws://[::fdca:251.251.254.83] < / ```  <br> ``` ws://[::]?! < / ```  <br> ``` ws://[::cfed:254.251.89.255] ```  <br> ``` ws://[::] < /[풂 ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: u ```  |  ``` U:/ < //[::dbae:acfd:251.254.8.252] ```  <br> ``` U:/ < //[::badd:255.251.255.138] ```  <br> ``` U://-p ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: t ```  |  ``` t:/ < //T ```  <br> ``` T:/ < //%2e ```  <br> ``` t:/ < //[cfae::dbff] ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: f ```  |  ``` F:/ < //~! ```  <br> ``` f:/ < //[acde:dcab:beae:dfdb:abbb:fbcb:d7ce:fdae] ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: z ```  |  ``` z://[::dfbe:afea:251.254.251.254] < / ```  <br> ``` z://[abce::] < / ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: w ```  |  ``` W:/ < //[db4d::fdfd:cecb:250.254.251.254] ```  <br> ``` w:/ < //[::aeae:fcae:86.253.250.253] ```  <br> 
...
### chromium
 ...
 Exception Type | URLs 
 --- | --- 
 ``` invalid URL ```  |  ``` F:/ < //~! ```  <br> ``` J:/ < //; ```  <br> ``` X://M < / ```  <br> ``` v://[ceda:eaab:cafc:afbb:bfac:fdaf:254.253.253.253] < / ```  <br> ``` B://[::deee:202.255.253.255] < / ```  <br> ``` z://[::dfbe:afea:251.254.251.254] < / ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` https://;~ ```  <br> ``` o:/// < / ```  <br> ``` z://[abce::] < / ```  <br> ``` o://[::5bde] < / ```  <br> 
