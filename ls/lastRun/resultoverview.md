...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: z ```  |  ``` Z:/ < /#% ```  <br> ``` z://[::dfbe:afea:251.254.251.254] < / ```  <br> ``` z://[abce::] < / ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: ws ```  |  ``` ws://[::]/;횘 < / ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` ws://[9eba::bdcc] < / ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` ws://[::] < //[::babe:dcfb:ed7d:abbd] ```  <br> ``` ws://[::]/,% < / ```  <br> ``` ws://[::] < //[::daaf:cfca:255.22.254.252] ```  <br> ``` ws://[::] < V ```  <br> ``` ws://[caf7::255.252.252.251] ```  <br> ``` ws://[::fdca:251.251.254.83] < / ```  <br> ``` ws://[::]?! < / ```  <br> ``` ws://[::]/)︒ ```  <br> ``` ws://[::cfed:254.251.89.255] ```  <br> ``` ws://[::] < /[풂 ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: x ```  |  ``` X://[d1db:aace::] ```  <br> ``` X://[e5da:febd::fcad:252.251.252.254] ```  <br> ``` X://M < / ```  <br> ``` x:/ < //C ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: j ```  |  ``` J:/ < //; ```  <br> ``` j:/ < //[efbb:efbc:eaff:cbaa:aebf:cada:254.67.255.251] ```  <br> ``` J:/ < / ```  <br> 
