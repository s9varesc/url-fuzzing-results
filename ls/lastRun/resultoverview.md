...
### C
 ...
 Exception Type | URLs 
 --- | --- 
 ``` 1 ```  |  ``` http://[::] < @𸻾 ```  <br> ``` file:/// < /#Ｓ ```  <br> ``` ftp://[::] < /?򯹨 ```  <br> ``` B://[::deee:202.255.253.255] ```  <br> ``` https://[::] < /#[ ```  <br> ``` Z:/ < /#% ```  <br> ``` file:///? ```  <br> ``` wss://[::]/./ ```  <br> ``` V:/ < /[ ```  <br> ``` file:/// < /% ```  <br> ``` file:/// < //{y ```  <br> ``` file:///#^ ```  <br> ``` F:/ < 쵖 ```  <br> ``` ws://[::]/;횘 ```  <br> ``` file:/// < /&ఒ ```  <br> ``` N:/ < /&￠ ```  <br> ``` o:/ < ￹ ```  <br> ``` file:///%7b￧ ```  <br> ``` https://[::]#ﾛ ```  <br> ``` http://[::] < / ```  <br> ``` file://[::]/ꪸ ```  <br> ``` file:///#󥓈 ```  <br> ``` file:/// < /%2e//[ ```  <br> ``` file:/// < /? ```  <br> ``` file://`$ ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` file:///鍕 ```  <br> ``` https://[::]/﫞 ```  <br> ``` file:/// < //"* ```  <br> ``` file:/// < ///|񁣀 ```  <br> ``` file:/// < ︢ ```  <br> ``` ws://[::] < /|, ```  <br> ``` ftp://[::] < / ```  <br> ``` wss://􀕻@+ ```  <br> ``` file:///%7b􏕒 ```  <br> ``` R:/[񂗉 ```  <br> ``` file:/// < / ```  <br> ``` http://[::] < /?텫 ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` L:/ < ///.// ```  <br> ``` file://[::]/../􏳁 ```  <br> ``` ws://[::] < /!އ ```  <br> ``` t:/? ```  <br> ``` file:/// < %7dＥ ```  <br> ``` O:/ < %2e%2e/// ```  <br> ``` file:///%%22 ```  <br> ``` file:/// < //ċz ```  <br> ``` wss://Gſ ```  <br> ``` file:/// < //[::]/./鹷 ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` http://[::] < //[::]/%60ａ ```  <br> ``` file://[::]/%2e%2e/󬱰 ```  <br> ``` ws://[::]/,% ```  <br> ``` http://[::] <  ```  <br> ``` file:/// < 햐 ```  <br> ``` H:/#􏨊 ```  <br> ``` file:/// < .%2e///[ ```  <br> ``` https://[::]?㡑 ```  <br> ``` http://[::] < /?ퟻ ```  <br> ``` file:/// < /.///ᶓ ```  <br> ``` ftp://[::] < ./񾣩 ```  <br> ``` file://" ```  <br> ``` r:/ < /# ```  <br> ``` file:/// < /%2e/% ```  <br> ``` file:///#틣 ```  <br> ``` http://[::] < ퟺ ```  <br> ``` x:/ < ///'￪ ```  <br> ``` file:/// < /.////쇕 ```  <br> ``` P:/../[ ```  <br> ``` A:/#隤 ```  <br> ``` ws://[::] < /# ```  <br> ``` W:/ < /// ```  <br> ``` q:/ < ..//󜐢 ```  <br> ``` https://[::] < 放 ```  <br> ``` S:/(ﻴ ```  <br> ``` t:/ < /$񳗓 ```  <br> ``` file://[::]/%2e.//ﻍ ```  <br> ``` file:/// < //[::]/.//㶭 ```  <br> ``` file:/// < /崍 ```  <br> ``` file:///^ ```  <br> ``` https://[::] < ퟾ ```  <br> ``` file:/// < ///=ｿ ```  <br> ``` ftp://[::eecc:255.252.255.207] ```  <br> ``` https://[::] < //%@) ```  <br> ``` file:///#鵩 ```  <br> ``` H:/ < %2e%2e/𧀷 ```  <br> ``` Z:///,􏛖 ```  <br> ``` h:/񻿼 ```  <br> ``` ws://[::]/)︒ ```  <br> ``` J:/ ```  <br> ``` file:/// < /# ```  <br> ``` Y:/ < %7b￸ ```  <br> ``` z:/ < /ﾚ ```  <br> ``` M:/ < /+ ```  <br> ``` wss://[::]?Ҽ ```  <br> ``` wss://[::] < /𰵮 ```  <br> ``` L:/ < ///^. ```  <br> ``` http://[::]?򅴄 ```  <br> ``` b:/? ```  <br> ``` https://[::] < 􌧄 ```  <br> ``` V:///]! ```  <br> ``` file:/// < //[::]/ﺑ ```  <br> ``` ws://@~ ```  <br> ``` H://" ```  <br> ``` http://[::] < %2e//󠱎 ```  <br> ``` B:/ < /#︓ ```  <br> ``` c:/ < /ﮢ ```  <br> ``` http://[::] < //󢯼@= ```  <br> ``` s:/ < /?啁 ```  <br> ``` https://[::] < //M@` ```  <br> ``` file:///# ```  <br> ``` Y:///%2e./| ```  <br> ``` G:/%7bᡚ ```  <br> ``` http://[::]?ퟻ ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> ``` http://[::]/%60𓜎 ```  <br> ``` ftp://[::] < ﹐ ```  <br> ``` z:/ < //$} ```  <br> ``` file:///#򮠱 ```  <br> ``` http://[::]?沫 ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` ftp://[::] < 􏟃 ```  <br> ``` file:///#⮃ ```  <br> ``` p:/?ﺖ ```  <br> ``` wss://[::] < //ſ3 ```  <br> ``` file:///'＿ ```  <br> ``` file://}a ```  <br> ``` file://[::]/酜 ```  <br> ``` file:/// < //}` ```  <br> ``` file:/// < //[::]/%2e.//痁 ```  <br> ``` r:/| ```  <br> 
...
### Java
 ...
 ``` java.net.MalformedURLException: unknown protocol: a ```  |  ``` A:/ < //[::aafe:d3dc:bddf] ```  <br> ``` a:/ < //[aebb:cfee:ddba:cbda:fbca:aebc:b6cd:dcba] ```  <br> ``` A:/ < //[::adbf:dedc:252.254.44.255] ```  <br> ``` A:/ < ///%2e%2e//%2e ```  <br> ``` A:/ < //[::e9cf:cdce] ```  <br> ``` A:/P:/ < / ```  <br> ``` A:/#隤 < / ```  <br> ``` a:/ < //%6f ```  <br> ``` a:/ < //[::251.253.251.215] ```  <br> ``` A:/ < //[::aeb6] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: f ```  |  ``` F:/ < 쵖 ```  <br> ``` F:/ < //~! ```  <br> ``` F:/#= < / ```  <br> ``` f:/ < //[acde:dcab:beae:dfdb:abbb:fbcb:d7ce:fdae] ```  <br> ``` F:/ < %2e%2e/򀒉 ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: ws ```  |  ``` ws://[::]/;횘 < / ```  <br> ``` ws://[::] < //:K@" ```  <br> ``` ws://[::]/.%2e < / ```  <br> ``` ws://[9eba::bdcc] < / ```  <br> ``` ws://[::] < /|, ```  <br> ``` ws://[::] < //[::acef:deac:203.253.252.251] ```  <br> ``` ws://[::] < /!އ ```  <br> ``` ws://[::] < //[::babe:dcfb:ed7d:abbd] ```  <br> ``` ws://O ```  <br> ``` ws://[::]/,% < / ```  <br> ``` ws://[::] < //[::daaf:cfca:255.22.254.252] ```  <br> ``` ws://[::] < V ```  <br> ``` ws://[caf7::255.252.252.251] ```  <br> ``` ws://[::] < /# ```  <br> ``` ws://[::fdca:251.251.254.83] < / ```  <br> ``` ws://[::]?! < / ```  <br> ``` ws://[::]/)︒ ```  <br> ``` ws://[::cfed:254.251.89.255] ```  <br> ``` ws://[::] < //[ba5b:ebad::fceb:255.252.251.250] ```  <br> ``` ws://[::bada:aecd:250.78.252.252] ```  <br> ``` ws://@~ ```  <br> ``` ws://[::]:65511 < / ```  <br> ``` ws://[::] < /[풂 ```  <br> ``` ws://[::] < U ```  <br> ``` ws://[fc3b::] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: p ```  |  ``` P:/ < //[::cfcc:253.88.251.253] ```  <br> ``` P:/. ```  <br> ``` P:/ < //[::fbaa:ceec:250.254.149.251] ```  <br> ``` P:/../[ ```  <br> ``` p:/ < //%7d ```  <br> ``` p:/?ﺖ < / ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: e ```  |  ``` e:/ < //[::7bce:fdcc] ```  <br> ``` e:/ < %2e////= ```  <br> ``` E:/%60( ```  <br> ``` E:/N:/ ```  <br> ``` e:/ < //[::253.254.44.255] ```  <br> ``` e:/ < //[fafe:cacb:edff:faea:defa:bddc:85.252.250.251] ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: i ```  |  ``` i:/ < //[::f7fa:dafa] ```  <br> ``` i:/ < //[::5fbf:bdbc] ```  <br> ``` I:/ < /%60* ```  <br> 
 ...
 ``` java.net.MalformedURLException: unknown protocol: w ```  |  ``` w:/ < //%5d ```  <br> ``` w://[::]:65516 ```  <br> ``` W:/ < /// ```  <br> ``` W:/ < //[db4d::fdfd:cecb:250.254.251.254] ```  <br> ``` w:/ < //[::aeae:fcae:86.253.250.253] ```  <br> ``` W:/ < /.//%2e%2e ```  <br> 
 ``` java.net.MalformedURLException: unknown protocol: g ```  |  ``` G:/ < //[::fdfa:efbf:caab:1dea] ```  <br> ``` g:/ < //[::acbe:bece:252.99.251.254] ```  <br> ``` G:/%7bᡚ ```  <br> ``` G:/ < //[::abce:eabd:252.6.250.252] ```  <br> 
...
### chromium
 ...
 Exception Type | URLs 
 --- | --- 
 ``` invalid URL ```  |  ``` F:/ < //~! ```  <br> ``` l://[bffb:ddcd::cbae:dffb:253.250.251.252] < / ```  <br> ``` J:/ < //; ```  <br> ``` X://M < / ```  <br> ``` X://r < / ```  <br> ``` L:/ < ///^. ```  <br> ``` v://[ceda:eaab:cafc:afbb:bfac:fdaf:254.253.253.253] < / ```  <br> ``` B://[::deee:202.255.253.255] < / ```  <br> ``` C:/ < //;0 ```  <br> ``` q://[daf9:dcce::dbcf:fbdc:255.250.254.252] < / ```  <br> ``` z://[::dfbe:afea:251.254.251.254] < / ```  <br> ``` d:/ < ///􏾐 ```  <br> ``` https://;~ ```  <br> ``` W:/ < /// ```  <br> ``` o:/// < / ```  <br> ``` z://[abce::] < / ```  <br> ``` w:/ < //%5d ```  <br> ``` file://;~ < / ```  <br> ``` o://[::5bde] < / ```  <br> ``` ws://@~ ```  <br> ``` https://[::] < //:S@~ ```  <br> 
